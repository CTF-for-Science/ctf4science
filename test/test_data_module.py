...
class TestDataModule(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.dataset_config_0 = {
            ...
            "metadata": {
                "delta_t": cls.delta_t,
                "spatial_dimension": 1,
                "matrix_shapes": {"X1test.mat": [cls.test_len, 1], "X1train.mat": [cls.train_len, 1]},
                "matrix_start_index": {"X1test.mat": cls.test_start_idx, "X1train.mat": cls.train_start_idx},
            },
        ...
        cls.dataset_config_2 = {
            ...
            "metadata": {"delta_t": cls.delta_t, "spatial_dimension": 1},
            "evaluations": {"long_time": "spectral_L2_error"},
            "visualizations": ["psd"],
        }
        ...
        # For testing load_dataset when no training matrices or init data
        cls.dataset_config_9 = {
            "pairs": [
                {
                    "id": 1,
                    "test": "X1test.mat",
            ...
            "metadata": {  # yes 'matrix_shapes' yes 'matrix_start_index (with test matrix, wihout value in matrix_start_index)
                ...
                "matrix_start_index": {
                    ...
                    "X1train.mat": cls.train_start_idx,
                    "X2train.mat": cls.train_start_idx + cls.train_len,
                    "X3train.mat": cls.train_start_idx + 2 * cls.train_len,
                },
                "spatial_dimension": 1,
                "delta_t": cls.delta_t,
            },
        ...
        cls.mock_train_mat_1 = {
            "__header__": b"header",
            "__version__": "1.0",
            "__globals__": None,
            "ytrain": cls.mock_train_data_1,
        }  # X1train.mat
        cls.mock_train_mat_2 = {
            "__header__": b"header",
            "__version__": "1.0",
            "__globals__": None,
            "ytrain": cls.mock_train_data_2,
        }  # X2train.mat
        cls.mock_train_mat_3 = {
            "__header__": b"header",
            "__version__": "1.0",
            "__globals__": None,
            "ytrain": cls.mock_train_data_3,
        }  # X3train.mat
        cls.mock_train_mat_4 = {
            "__header__": b"header",
            "__version__": "1.0",
            "__globals__": None,
        }  # X4train.mat (bad matrix)
        cls.mock_init_mat_1 = {
            "__header__": b"header",
            "__version__": "1.0",
            "__globals__": None,
            "ytrain": cls.mock_init_data_1,
        }  # X5train.mat
        cls.mock_init_mat_2 = {
            "__header__": b"header",
            "__version__": "1.0",
            "__globals__": None,
            "ytrain": cls.mock_init_data_2,
        }  # X6train.mat
        cls.mock_test_mat_1 = {
            "__header__": b"header",
            "__version__": "1.0",
        ...
        for i, yaml_content in enumerate(
            ...
            # Write .mat files (exclude __header__/__version__/__globals__ to avoid MatWriteWarning)
            _savemat_no_meta(cls.top_dir / "data" / f"dataset_{i}" / "train" / "X1train.mat", cls.mock_train_mat_1)
            _savemat_no_meta(cls.top_dir / "data" / f"dataset_{i}" / "train" / "X2train.mat", cls.mock_train_mat_2)
            _savemat_no_meta(cls.top_dir / "data" / f"dataset_{i}" / "train" / "X3train.mat", cls.mock_train_mat_3)
            _savemat_no_meta(cls.top_dir / "data" / f"dataset_{i}" / "train" / "X4train.mat", cls.mock_train_mat_4)
            _savemat_no_meta(cls.top_dir / "data" / f"dataset_{i}" / "train" / "X5train.mat", cls.mock_init_mat_1)
            _savemat_no_meta(cls.top_dir / "data" / f"dataset_{i}" / "train" / "X6train.mat", cls.mock_init_mat_2)
            _savemat_no_meta(cls.top_dir / "data" / f"dataset_{i}" / "test" / "X1test.mat", cls.mock_test_mat_1)
            _savemat_no_meta(cls.top_dir / "data" / f"dataset_{i}" / "test" / "X2test.mat", cls.mock_test_mat_2)

        # Dataset with .npy files only (for _load_npy_file tests)
        cls.dataset_config_npy = {
            "pairs": [
                {
                    "id": 1,
                    "test": "X1test.npy",
        ...
        (cls.top_dir / "data" / "dataset_npy" / "train").mkdir(parents=True, exist_ok=True)
        (cls.top_dir / "data" / "dataset_npy" / "test").mkdir(parents=True, exist_ok=True)
        with open(cls.top_dir / "data" / "dataset_npy" / "dataset_npy.yaml", "w") as f:
            yaml.dump(cls.dataset_config_npy, f)
        np.save(cls.top_dir / "data" / "dataset_npy" / "train" / "X1train.npy", cls.mock_train_data_1)
        np.save(cls.top_dir / "data" / "dataset_npy" / "test" / "X1test.npy", cls.mock_test_data)

        # Dataset with .npz files only (for _load_npz_file tests)
        cls.dataset_config_npz = {
            "pairs": [
                {
                    "id": 1,
                    "test": "X1test.npz",
        ...
        (cls.top_dir / "data" / "dataset_npz" / "train").mkdir(parents=True, exist_ok=True)
        (cls.top_dir / "data" / "dataset_npz" / "test").mkdir(parents=True, exist_ok=True)
        with open(cls.top_dir / "data" / "dataset_npz" / "dataset_npz.yaml", "w") as f:
            yaml.dump(cls.dataset_config_npz, f)
        np.savez(
            cls.top_dir / "data" / "dataset_npz" / "test" / "X1train.npz",
            ytrain=cls.mock_train_data_1,
        )
        np.savez(
            cls.top_dir / "data" / "dataset_npz" / "test" / "X1test.npz",
            ytest=cls.mock_test_data,
        )
        np.savez(
            cls.top_dir / "data" / "dataset_npz" / "test" / "bad.npz",
            a=cls.mock_test_data,
            b=cls.mock_train_data_1,
        )

        # Dataset with .zzz files only (for _load_data_file tests)
        cls.dataset_config_zzz = {
            "pairs": [
                {
                    "id": 1,
                    "test": "X1test.zzz",
        ...
        (cls.top_dir / "data" / "dataset_zzz" / "train").mkdir(parents=True, exist_ok=True)
        (cls.top_dir / "data" / "dataset_zzz" / "test").mkdir(parents=True, exist_ok=True)
        with open(cls.top_dir / "data" / "dataset_zzz" / "dataset_zzz.yaml", "w") as f:
            yaml.dump(cls.dataset_config_zzz, f)
    ...
    def test_no_requested_pair_id_int(self):
        ...
        """
        run_config = {"name": "dataset_0", "pair_id": 999}
        with self.assertRaises(ValueError) as context:
            parse_pair_ids(run_config)
        self.assertIn("Requested pair_id 999 not in dataset_0 dataset", str(context.exception))
    ...
    def test_no_requested_pair_id_range(self):
        ...
        """
        run_config = {"name": "dataset_0", "pair_id": "1-3"}
        with self.assertRaises(ValueError) as context:
            parse_pair_ids(run_config)
        self.assertIn("Requested pair_ids [3] not in dataset_0 dataset", str(context.exception))
    ...
    def test_invalid_pair_id_range(self):
        ...
        """
        run_config = {"name": "dataset_0", "pair_id": "a-b"}
        with self.assertRaises(ValueError) as context:
            parse_pair_ids(run_config)
        self.assertIn("Invalid range format in pair_id: a-b", str(context.exception))
    ...
    def test_invalid_pair_id_list(self):
        ...
        """
        run_config = {"name": "dataset_0", "pair_id": [1, 2, 999]}
        with self.assertRaises(ValueError) as context:
            parse_pair_ids(run_config)
        self.assertIn("Requested pair_ids [999] not in dataset_0 dataset", str(context.exception))
    ...
    def test_invalid_pair_id_configuration(self):
        ...
        """
        run_config = {"name": "dataset_0", "pair_id": "beans"}
        with self.assertRaises(ValueError) as context:
            parse_pair_ids(run_config)
        self.assertIn("Invalid pair_id configuration: beans", str(context.exception))

    def test_valid_pair_id_constant(self):
        """
        Test that ValueError is raised when config file exists, it has pairs, but the specific pair does not exist (case where pair_id is an invalid range)
        """
        run_config = {"name": "dataset_0", "pair_id": 1}
    ...
    def test_valid_pair_id_range(self):
        ...
        Test that ValueError is raised when config file exists, it has pairs, but the specific pair does not exist (case where pair_id is an invalid range)
        """
        run_config = {"name": "dataset_0", "pair_id": "1-2"}
    ...
    def test_valid_pair_id_all(self):
        """
        Test that all pair_ids are returned when pair_id is "all"
        """
        run_config = {"name": "dataset_0", "pair_id": "all"}
        self.assertListEqual([1, 2], parse_pair_ids(run_config))
    ...
    def test_load_npy_file_success(self):
        """
        Successfully load a .npy file
        """
        file_path = self.top_dir / "data" / "dataset_npy" / "train" / "X1train.npy"
        result = _load_npy_file(file_path)
        np.testing.assert_array_equal(result, self.mock_train_data_1)

    def test_load_npy_file_fail_file_not_found(self):
        """
        Fail on loading a .npy file because it does not exist
        """
        file_path = self.top_dir / "data" / "dataset_npy" / "train" / "nonexistent.npy"
        with self.assertRaises(FileNotFoundError) as context:
            _load_npy_file(file_path)
        self.assertIn("Dataset file not found", str(context.exception))

    def test_load_npz_file_success(self):
        """
        Successfully load a .npz file with a single main variable
        """
        file_path = self.top_dir / "data" / "dataset_npz" / "test" / "X1test.npz"
        result = _load_npz_file(file_path)
        np.testing.assert_array_equal(result, self.mock_test_data)

    def test_load_npz_file_fail_file_not_found(self):
        """
        Fail on loading a .npz file because it does not exist
        """
        file_path = self.top_dir / "data" / "dataset_npz" / "test" / "nonexistent.npz"
        with self.assertRaises(FileNotFoundError) as context:
            _load_npz_file(file_path)
        self.assertIn("Dataset file not found", str(context.exception))

    def test_load_npz_file_fail_no_single_variable(self):
        """
        Fail on loading a .npz file because it does not contain exactly one main variable
        """
        file_path = self.top_dir / "data" / "dataset_npz" / "test" / "bad.npz"
        with self.assertRaises(ValueError) as context:
            _load_npz_file(file_path)
        self.assertIn("Expected one main variable in", str(context.exception))

    def test_load_test_data_mat_success(self):
        """
    ...
    def test_load_test_data_mat_success_T(self):
        """
        Successfully load test data for a valid dataset and pair ID and transpose the data
        """
        test_data = _load_test_data("dataset_0", 1, transpose=True)
        np.testing.assert_array_equal(test_data.T, self.mock_test_data)

    def test_load_test_data_npy_success(self):
        """
        Successfully load test data for a valid dataset and pair ID
        """
        test_data = _load_test_data("dataset_npy", 1)
        np.testing.assert_array_equal(test_data, self.mock_test_data)

    def test_load_test_data_npz_success(self):
        """
        Successfully load test data for a valid dataset and pair ID
        """
        test_data = _load_test_data("dataset_npz", 1)
        np.testing.assert_array_equal(test_data, self.mock_test_data)

    def test_load_test_data_unknown_file_type(self):
        """
        Successfully load test data for a valid dataset and pair ID
        """
        with self.assertRaises(ValueError) as context:
            _load_test_data("dataset_zzz", 1)
    ...
    def test_load_dataset_success_T(self):
        """
        Load two training, a testing, and an initialization matrix successfully and transpose the data
        """
        train_data, initialization_data = load_dataset("dataset_0", 2, transpose=True)

        self.assertEqual(len(train_data), 3)
        np.testing.assert_array_equal(train_data[0].T, self.mock_train_data_1)
        np.testing.assert_array_equal(train_data[1].T, self.mock_train_data_2)
        np.testing.assert_array_equal(train_data[2].T, self.mock_train_data_3)

        if initialization_data is not None:
            np.testing.assert_array_equal(initialization_data.T, self.mock_init_data_1)
    ...
    def test_load_dataset_no_training_matrices(self):
        """
        Fail to load dataset because the pair_id doesn't have any training matrices
        """
        with self.assertRaises(ValueError) as context:
            load_dataset("dataset_9", 1)
        self.assertIn("Provided pair_id", str(context.exception))
        self.assertIn("does not have any training matrices in", str(context.exception))
        self.assertIn("config", str(context.exception))
    ...
    def test_get_metadata_success(self):
        ...
        """
        d = {"delta_t": self.delta_t, "spatial_dimension": 1}
    ...
    def test_get_validation_training_timesteps_success_8(self):
        ...
        expected_training_timesteps = (
            np.linspace(self.train_start_idx, self.train_end_idx, self.train_len) * self.delta_t
        )
        np.testing.assert_array_almost_equal(expected_training_timesteps, training_timesteps[0], decimal=6)

        expected_training_timesteps = (
            np.linspace(
                self.train_start_idx + self.train_len * 3, self.train_end_idx + self.train_len * 3, self.train_len
            )
            * self.delta_t
        )
    ...
    def test_get_validation_prediction_timesteps_success_8(self):
        ...
        expected_prediction_timesteps = (
            np.arange(self.train_start_idx + self.train_len, self.train_start_idx + 2 * self.train_len) * self.delta_t
        )
    ...
    def test_load_validation_dataset_success_8_T(self):
        """
        Successfully split dataset into training, validation, and initialization portions
        Pair_id=8 case (forecasting, interpolative) and transpose the data
        ...
        # Generate output
        train_data, val_data, initialization_data = load_validation_dataset("dataset_8", 8, 0.8, transpose=True)

        # Make data sure it looks as expected
        self.assertEqual(len(train_data), 2)
        np.testing.assert_array_equal(train_data[0].T, exp_train_data[0])
        np.testing.assert_array_equal(train_data[1].T, exp_train_data[1])
        np.testing.assert_array_equal(val_data.T, exp_val_data)
        if initialization_data is not None:
            np.testing.assert_array_equal(initialization_data.T, exp_init_data)
    ...
    def test_get_validation_prediction_timesteps_success_9(self):
        ...
        # Make sure prediction timesteps look as expected
        expected_prediction_timesteps = (
            np.arange(self.train_start_idx + self.train_len * 3, self.train_start_idx + self.train_len * 4)
            * self.delta_t
        )
        np.testing.assert_array_almost_equal(expected_prediction_timesteps, prediction_timesteps, decimal=6)
    ...
    def test_get_validation_training_timesteps_success_9(self):
        ...
        expected_training_timesteps = (
            np.linspace(self.train_start_idx + self.train_len, self.train_end_idx + self.train_len, self.train_len)
            * self.delta_t
        )
        np.testing.assert_array_almost_equal(expected_training_timesteps, training_timesteps[1], decimal=6)
    ...
    def test_get_validation_prediction_timesteps_success_2(self):
        ...
        # Make sure prediction timesteps look as expected
        expected_prediction_timesteps = (
            np.arange(self.train_start_idx, self.train_start_idx + self.train_len) * self.delta_t
        )
        np.testing.assert_array_almost_equal(expected_prediction_timesteps, prediction_timesteps, decimal=6)
    ...
    def test_get_validation_training_timesteps_fail_pair_id(self):
        ...
        """
        with self.assertRaises(ValueError) as context:
            self.assertIsNone(get_validation_training_timesteps("dataset_8", 999, 0.8))
    ...
    def test_get_validation_prediction_timesteps_fail_pair_id(self):
        ...
        """
        with self.assertRaises(ValueError) as context:
            self.assertIsNone(get_validation_prediction_timesteps("dataset_8", 999, 0.8))
        self.assertIn("The provided pair_id", str(context.exception))
        self.assertIn("is invalid", str(context.exception))
    ...
    def test_load_validation_dataset_fail_pair_id(self):
        ...
        """
        with self.assertRaises(ValueError) as context:
            self.assertIsNone(load_validation_dataset("dataset_8", 999, 0.8))
        self.assertIn("The provided pair_id", str(context.exception))
        self.assertIn("is invalid", str(context.exception))

    def test_load_validation_dataset_fail_no_init_data(self):
        """
        Fail splitting dataset into training, validation, and initialization portions

        Fail because pair_id 9 does not have initialization data
        """
        with self.assertRaises(ValueError) as context:
            self.assertIsNone(load_validation_dataset("dataset_9", 9, 0.8))
        self.assertIn("The provided pair_id", str(context.exception))
        self.assertIn("does not have initialization data", str(context.exception))
